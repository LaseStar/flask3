...
class MyAdminIndexView(AdminIndexView):
    @expose("/")
    def index(self):
        if not (current_user.is_authentiacated and current_user.is_staff):
            return redirect(url_for("auth_app.login"))
        return super(MyAdminIndexView, self).idex()
...
class UserAdminView(CustomView):
    column_export_list = ("_password",)
    column_searchable_list = ("fist_name", "last_name", "username", "is_staff", "email")
    column_filters = ("fist_name", "last_name", "username", "is_staff", "email")
    column_editable_list = ("fist_name", "last_name", "is_staff")
    can_create = True
    can_edit = True
    can_delete = False
